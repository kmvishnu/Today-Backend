...
export const login = async (req, res) => {
    ...
    try{
        ...
        const userRows:any[] = await Config.MySqlDB.query('SELECT id,name,password FROM testUsers WHERE email =:email ',
...
export const signUp = async (req,res)=>{
    ...
    try{
        ...
        const existingUserRows = await Config.MySqlDB.query('SELECT id FROM testUsers WHERE email =:email ',
        { replacements : { 'email': email },type:QueryTypes.SELECT});
        ...
        if (existingUserRows.length > 0) {
          ...
          await Config.MySqlDB.query('INSERT INTO testUsers (name, email, password) VALUES (:name, :email, :password)',
